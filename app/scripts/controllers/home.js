...
angular.module('myApp.controllers')
	.controller( 'HomeController', ['$scope', '$http', '$location', '$timeout', 'authService', 
		'Socket','AppRegistry', 'PathFinder', 'LineOfSight', 'MoveManager', 'TileDataService',
	function($scope, $http, $location, $timeout, authService, Socket, AppRegistry, PathFinder, LineOfSight, MoveManager, TileDataService){
		...
		$scope.$on('onMouseOverMapChanged', function(evt, col, row){
			...
			if(AppRegistry.map[row][col].visibility == 0 || AppRegistry.map[row][col] == undefined){
				$scope.tileDesc = null;
				$scope.$apply();
				return;
			}
			...
			$scope.tileDesc = TileDataService.getDesc(AppRegistry.map[row][col]);
